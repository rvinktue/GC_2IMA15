...
#colors in indices
def _debug_draw_graph(G, colors = []):
...
t0 = time.time_ns()
instance_name = "sqrp7730" #"reecn3382"
instance = read_instance("./instances/" + instance_name + ".instance.json")
print(f"Reading took: {(time.time_ns() - t0)/(10**6)} ms")
...
print(f"Naive graph generation took: {(time.time_ns() - t0)/(10**6)} ms")
...
for i in range(len(colours)):
    if (len(vertices[i]) != 0):
        ...
        #print(f"{i}: get's color {colours[i]}")


t0 = time.time_ns()
_debug_draw_graph(G,colours)
print(f"Debug draw graph took: {(time.time_ns() - t0)/(10**6)} ms")
